...
window.addEventListener('DOMContentLoaded', async function (event) {
  ...
  // ⬇️ ⬇️ ⬇️
  let response = await fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=2ce14a76fb543a32e59f7c3036a548d4&language=en-US`)
  ...
  for (let i = 0; i < moviesResults.length; i++) {
    ...
    let movie = moviesResults[i]
    let movieID = movie.id
    let posterpath = movie.backdrop_path
    //console.log(posterpath)


    let quereySnapshot = await db.collection('movies').get()
    let movieswatched = quereySnapshot.docs
    let temp = await db.collection('movies').doc(`${movieID}`).get()

    //let docRef = await db.collection(`movies`).doc(`${movieID}`).set({ text: 'I HAVE NOT WATCHED IT YET!' })
    let movieHtml = document.querySelector('.movies')
    movieHtml.insertAdjacentHTML('beforeend', `<div class="w-1/5 p-4 movie-${movieID}">
    ...
    if (temp.data()) {
      ...
      console.log('I HAVE NOT WATCHED IT YET!')
    ...
    document.querySelector(`.movie-${movieID} .watched-button`).addEventListener('click', async function (event) {
      ...
      //console.log(`movie ${movieID} like button clicked!`)

      document.querySelector(`.movie-${movieID}`).classList.add('opacity-20')

      //if (document.querySelector(`opacity-20`)){

      await db.collection(`movies`).doc(`${movieID}`).set({text: 'watched'})
